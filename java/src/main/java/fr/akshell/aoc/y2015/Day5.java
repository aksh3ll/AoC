...
public class Day5 extends BaseDay<Integer> {
    ...
    private static final Predicate<String> NICE_STRING_V1 =
    ...
    private static final Predicate<String> NICE_STRING_V2 =
            string -> RE_RULE1_V2.matcher(string).find()
                    && RE_RULE2_V2.matcher(string).find();

    public int countNiceStrings(Predicate<String> niceStringTest, String input) {
            return input
                    .lines()
                    .mapToInt(line -> niceStringTest.test(line) ? 1 : 0)
                    .sum();
    }
